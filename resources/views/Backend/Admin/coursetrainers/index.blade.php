...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-md-9 text-start fs-5">
            <a href="{{url('dashboard')}}">Dashboard</a> |
            <a href="{{url('categories')}}">Categories</a> |
            <a href="{{url('categories',$course->category_id)}}">{{$course->category->title}}</a> |
            <a >{{$course->title}}</a> |
            <a >Course Trainers</a>
        </div>
    ...
    <div class="modal fade" id="Add" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <form action="{{url('coursetrainers')}}" method="post" enctype="multipart/form-data" > @csrf
          <input type="hidden" name="course_id" value="{{$course->id}}">
          <div class="modal-body">
            <div class="row">
                <div class="col-12 mb-3">
                    <label for="trainer_id">Trainer</label>
                    <select name="trainer_id" class="form-select mt-2" required>
        ...
        <div class="col-12">
            ...
            <div class="table-responsive">
                <table class="table table-dark table-responsive table-hover table-striped">
                  ...
                  <tbody>
                    @foreach($coursetrainers as $data)
                    <tr>
                      <th scope="row">{{$loop->index+1}}</th>
                      <td>{{$data->trainer->name}}</td>
                      <td>
                          <img height="50" width="auto" src="{{asset('images/trainers/'.$data->trainer->image)}}" alt="image">
                      </td>
                    ...
                    <div class="modal fade" id="Edit{{$data->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                      <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                          ...
                          <form action="{{url('coursetrainers/'.$data->id)}}" method="post" enctype="multipart/form-data" > @csrf @method('put')
                          <div class="modal-body">
                            <div class="row">
                                <div class="col-12 mb-3">
                                    <label for="title">Title</label>
                                        <select name="trainer_id" class="form-select mt-2" required>
                                            <option value="{{$data->trainer_id}}" hidden>{{$data->trainer->name}}</option>
                    ...
                    <div class="modal fade" id="Delete{{$data->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                      <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                          ...
                          <form action="{{url('coursetrainers/'.$data->id)}}" method="post" enctype="multipart/form-data" > @csrf @method('delete')
                          <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
