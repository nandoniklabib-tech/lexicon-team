        ...
        <div class="col-12">
            ...
            <div class="table-responsive">
                <table class="table table-dark table-responsive table-hover table-striped">
                  ...
                  <tbody>
                    @foreach($examdatetests as $data)
                    <tr>
                      <th scope="row">{{$loop->index+1}}</th>
                      <td class="text-nowrap">{{$data->created_at->setTimezone('Asia/Dhaka')->format('d M Y - h:i A')}}</td>
                      <td>
                          {{$data->exam_title}}<br>
                          {{$data->exam_date}} - @if($data->exam_date_time){{ \Carbon\Carbon::createFromFormat('H:i', $data->exam_date_time)->format('g:i A') }}@endif<br>
                          {{$data->test_name}}<br>
                          {{$data->requirement_detail}}
                      </td>
                      <td>
                          {{$data->name}}<br>
                          {{$data->mobile}}<br>
                    ...
                    <div class="modal fade" id="Delete{{$data->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                      <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                          ...
                          <form action="{{url('examdatetests/'.$data->id)}}" method="post" enctype="multipart/form-data" > @csrf @method('delete')
